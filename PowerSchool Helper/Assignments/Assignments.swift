...
extension AssignmentViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let percent = Util.convertScoreToGrade(score: assignment.score)
        cell.percentLabel.text = percent.isNaN || percent.isInfinite ? "--" : "\(Int(percent))%"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        assignmentDetailVC.flagLabel.attributedText = Util.convertFlagToNameAndColor(flags: assignment.flags)
    }
...
class AssignmentCalculatorViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func addPointCalculator() {
        ...
        let pointTextField = UITextField(frame: CGRect(x: 10, y: yPos, width: view.frame.width-150, height: height))
        ...
        let action = UIAction(title: "title") { _ in
            let text = pointTextField.text ?? ""
            if text.isNumber {
                let points = Int(text) ?? 0
                pointTextField.text = ""
        ...
        let addButton = UIButton(frame: CGRect(x: view.frame.width-130, y: yPos, width: 120, height: height), primaryAction: action)
        addButton.setTitle("Calculate", for: .normal)
        let dark: UIColor = UIColor(red: 30/255, green: 30/255, blue: 30/255, alpha: 1)
        addButton.backgroundColor = dark
    ...
    func calculate(value: Int) {
        sections = []
        let total = DatabaseManager.shared.getClassData(classType: AccountManager.global.classType, type: .total) as! Float + Float(value)
        let recieved = DatabaseManager.shared.getClassData(classType: AccountManager.global.classType, type: .received) as! Float
...
class AllAssignmentsListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating {
    
    @IBOutlet weak var table: UITableView!
    var assignments = RealmSwift.List<Assignments>()
    var filteredAssignments = RealmSwift.List<Assignments>()
    ...
    override func viewDidAppear(_ animated: Bool) {
        assignments = DatabaseManager.shared.getAllAssignments()
        filteredAssignments = DatabaseManager.shared.getAllAssignments()
        if assignments.count == 0 {
            noAssignments()
        } else {
            if let label = view.viewWithTag(100) { label.removeFromSuperview() }
        }
        table.reloadData()
    }
    
    func noAssignments() {
        let label = UILabel(frame: CGRect(x: 10, y: 50, width: view.frame.width-20, height: 100))
        label.text = "No assignments found. Click on a Class to add assignments"
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        if text == "" { filteredAssignments = DatabaseManager.shared.getAllAssignments() }

        for assignment in assignments  {
            if !assignment.name.uppercased().contains(text.uppercased()) { continue }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        tableView.separatorColor = .darkGray
        let cell = table.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! AllAssignmentsTableCellView
        
        cell.nameLabel.text = "\(filteredAssignments[indexPath.row].name)"
        cell.scoreLabel.text = "\(filteredAssignments[indexPath.row].score)"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        assignmentDetailVC.dateLabel.text = assignment.date
        assignmentDetailVC.nameLabel.text = assignment.name
        assignmentDetailVC.scoreLabel.text = assignment.score
        assignmentDetailVC.percentLabel.text = "\(Util.convertScoreToGrade(score: assignment.score))%"
        assignmentDetailVC.categoryLabel.text = assignment.category
...
class AllAssignmentsTableCellView: UITableViewCell {
    @IBOutlet weak var nameLabel: UILabel!
    @IBOutlet weak var scoreLabel: UILabel!
